...
class MainController extends Controller
{
    ...
    public function quiz($slug){
        $quiz = Quiz::whereSlug($slug)->with('questions.my_answer', 'my_result')->first() ?? abort(404, 'Quiz tapilmadi');
        if($quiz->my_result){
    ...
    public function quiz_detail($slug){
        $quiz = Quiz::whereSlug($slug)->with('my_result', 'topTen.user')->withCount('questions')->first() ?? abort(404, 'Quiz tapilmadi');
        return view('quiz_detail', compact('quiz'));
    }

    public function result(Request $request, $slug){
        $quiz = Quiz::with('questions')->whereSlug($slug)->first() ?? abort(404, 'Quiz tapilmadi');
        $correct = 0;
        ...
        return redirect()->route('quiz.detail', $quiz->slug)->withSuccess("Test bitdi. Topladigin xal: ".$point);
    }
}
